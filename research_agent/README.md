# Tennis Research Agent Example

This example shows how you might compose a richer tennis research agent using the Agents SDK. The pattern is similar to the `research_bot` example, but with more specialized sub‑agents and a verification step.

The flow is:

1. **Planning**: A planner agent turns the end user’s request into a list of search terms relevant to tennis analysis – recent news, upcoming tournaments, head-to-head matches, player recent performance, etc.
...
```bash
python -m examples.tennis_research_agent.main
```
...
```
You are a senior tennis analyst. You will be provided with the original query
